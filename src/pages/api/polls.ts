...
// Lightweight validation: Checks if the raw MDX file actually contains the poll key
const validatePollPresence = async (slug: string, pollKey: string) => {
    const posts = await getCollection('posts');
    const post = posts.find((entry) => entry.id === slug);
    if (!post || !post.body) return false;
    
    return post.body.includes(pollKey);
};
...
export const POST: APIRoute = async ({ request, cookies }) => {
    ...
    const isValidPoll = await validatePollPresence(slug, pollKey);
    if (!isValidPoll) {
    ...
    // Sane upper bound since we removed the heavy AST option counter
    if (optionIndex > 20) {
